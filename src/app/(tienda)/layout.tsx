...
export default async function LayoutTienda({ children }: { children: React.ReactNode }) {
  ...
  const [{ data: config }, { data: promocion }, { data: perfil }] = await Promise.all([
    ...
    supabase.from('promociones')
      ...
      .eq('esta_activa', true)
      .or(`inicia_en.is.null,inicia_en.lte.${new Date().toISOString()}`)
      .or(`termina_en.is.null,termina_en.gte.${new Date().toISOString()}`)
