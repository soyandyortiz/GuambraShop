...
import {
  ...
  User, Phone, MessageCircle, X, ChevronDown, PartyPopper,
} from 'lucide-react'
...
export function VistaCalendario({ citas: citasInic, eventosConfirmados = [], mesActual }: Props) {
  ...
  // ── Eventos confirmados agrupados por día ─────────────────────────────────
  const eventosPorDia = useMemo(() => {
    const map: Record<number, EventoConfirmado[]> = {}
    eventosConfirmados.forEach(e => {
      if (!e.fecha_evento) return
      const dia = parseInt(e.fecha_evento.split('-')[2], 10)
      ;(map[dia] ??= []).push(e)
    })
    return map
  }, [eventosConfirmados])
  ...
  const eventosMes        = eventosConfirmados.length
  ...
  return (
    <div className="flex flex-col xl:flex-row gap-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
          {[
            ...
            { label: 'Eventos',        valor: eventosMes, color: 'text-purple-600'  },
          ].map(s => (
            ...
            {celdas.map((dia, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Barra de eventos confirmados */}
                  {(eventosPorDia[dia] ?? []).length > 0 && (
                    <div className="w-full mt-1">
                      <div className="h-1 bg-purple-500 rounded-full w-full" />
                      <span className="hidden sm:block text-[9px] text-purple-600 font-bold mt-0.5 leading-tight">
                        {(eventosPorDia[dia] ?? []).length} evento{(eventosPorDia[dia] ?? []).length !== 1 ? 's' : ''}
                      </span>
                    </div>
                  )}
        ...
        <div className="flex items-center gap-4 mt-3 px-1 flex-wrap">
          {[
            { color: 'bg-green-500',  label: 'Cita confirmada' },
          ...
          <div className="flex items-center gap-1.5">
            <span className="w-5 h-1 bg-purple-500 rounded-full flex-shrink-0" />
            <span className="text-[11px] text-foreground-muted">Evento confirmado</span>
          </div>
        </div>
      </div>
      ...
      <div className={cn(
              ...
              <>
                ...
                <p className="text-xs text-foreground-muted mt-0.5">
                  ...
                  {(eventosPorDia[diaSeleccionado!] ?? []).length > 0
                    ? <span className="text-purple-600 font-semibold"> · {(eventosPorDia[diaSeleccionado!] ?? []).length} evento{(eventosPorDia[diaSeleccionado!] ?? []).length !== 1 ? 's' : ''}</span>
                    : null
                  }
                </p>
              </>
        ...
        {/* Eventos confirmados del día */}
        {diaSeleccionado && (eventosPorDia[diaSeleccionado] ?? []).length > 0 && (
          <div className="border-b border-border bg-purple-50/50 px-3 py-2.5">
            <p className="text-[10px] font-extrabold text-purple-600 uppercase tracking-wide mb-2 flex items-center gap-1.5">
              <PartyPopper className="w-3 h-3" />
              Eventos confirmados ({(eventosPorDia[diaSeleccionado] ?? []).length})
            </p>
            <div className="flex flex-col gap-2">
              {(eventosPorDia[diaSeleccionado] ?? []).map(ev => (
                <div
                  key={ev.id}
                  className="bg-white border border-purple-200 rounded-xl px-3 py-2 flex items-start justify-between gap-2"
                >
                  <div className="min-w-0 flex-1">
                    <p className="text-[10px] font-bold text-purple-700 mb-0.5">{ev.numero_solicitud}</p>
                    <p className="text-xs font-semibold text-foreground truncate">{ev.nombre_cliente}</p>
                    <p className="text-[10px] text-foreground-muted truncate">{ev.producto_nombre}</p>
                  </div>
                  <a
                    href={`https://wa.me/${ev.whatsapp?.replace(/\D/g, '')}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-7 h-7 rounded-lg bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366]/20 flex items-center justify-center transition-all flex-shrink-0"
                    title="WhatsApp"
                  >
